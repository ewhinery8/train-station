...
Train Station uses a sophisticated cross-platform build system designed for zero external dependencies:

### Build Conventions
- **Target Matching**: Runtime target always matches compilation target (no cross-runtime deployment)
- **Static Linking**: CUDA FFI stubs statically linked for zero runtime dependencies
- **Cross-Platform**: Supports Windows (MSVC/MinGW), macOS (Clang/GCC), Linux (GCC/Clang)
- **Graceful Degradation**: Missing compilers result in warnings, not build failures

### Platform-Specific Builds
- **Windows**: `cl.exe` + `lib.exe` (MSVC) or `g++` + `ar` (MinGW) → static libraries
- **macOS**: `clang++` + `ar` → static libraries with `libc++` linking
- **Linux**: `g++` + `ar` → static libraries with `libstdc++` linking

### Feature Flags
```bash
cargo build                    # Zero dependencies (default)
cargo build --features cuda   # CUDA FFI stubs (experimental)
```

The build system automatically detects available toolchains and falls back gracefully, ensuring reliable builds across diverse CI/CD environments.
