//! Division operation performance benchmark
//!
//! This benchmark tests the performance of tensor division operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::ops::DivPerformanceTester, PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running Division Performance Benchmark");
    println!("=====================================");
    ...
    let mut tester = DivPerformanceTester::with_config(config);
    ...
    // Run all division performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("div_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: div_performance.json");
