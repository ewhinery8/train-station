//! Transpose operation performance benchmark
//!
//! This benchmark tests the performance of tensor transpose operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::transform::TransposePerformanceTester,
    PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running Transpose Performance Benchmark");
    println!("======================================");
    ...
    let mut tester = TransposePerformanceTester::with_config(config);
    ...
    // Run all transpose performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("transpose_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: transpose_performance.json");
