//! View operation performance benchmark
//!
//! This benchmark tests the performance of tensor view operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::transform::ViewPerformanceTester, PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running View Performance Benchmark");
    println!("==================================");
    ...
    let mut tester = ViewPerformanceTester::with_config(config);
    ...
    // Run all view performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("view_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: view_performance.json");
