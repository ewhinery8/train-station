//! Subtraction operation performance benchmark
//!
//! This benchmark tests the performance of tensor subtraction operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::ops::SubPerformanceTester, PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running Subtraction Performance Benchmark");
    println!("========================================");
    ...
    let mut tester = SubPerformanceTester::with_config(config);
    ...
    // Run all subtraction performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("sub_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: sub_performance.json");
