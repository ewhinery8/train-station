//! Addition operation performance benchmark
//!
//! This benchmark tests the performance of tensor addition operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::ops::AddPerformanceTester, PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running Addition Performance Benchmark");
    println!("=====================================");
    ...
    let mut tester = AddPerformanceTester::with_config(config);
    ...
    // Run all addition performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("add_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: add_performance.json");
