//! Flatten operation performance benchmark
//!
//! This benchmark tests the performance of tensor flatten operations
...
use libtorch_validation::performance::{
    generate_test_shapes_with_config, tensor::transform::FlattenPerformanceTester,
    PerformanceConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Running Flatten Performance Benchmark");
    println!("====================================");
    ...
    let mut tester = FlattenPerformanceTester::with_config(config);
    ...
    // Run all flatten performance tests with custom shapes
    let results = tester.test_all_operations_with_shapes(&test_shapes);

    // Save results to common JSON file
    tester.tester().save_results("flatten_performance.json")?;

    println!("\nBenchmark completed successfully!");
    println!("Results saved to: flatten_performance.json");
