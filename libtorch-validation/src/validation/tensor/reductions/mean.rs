...
impl TensorValidator {
    pub fn test_mean_forward(&self, shape: &[usize]) -> ComparisonResult {
    ...
    pub fn test_mean_gradients(&self, shape: &[usize]) -> ComparisonResult {
        let size: usize = shape.iter().product();
        ...
        let torch_m = match torch_y.mean() {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
    ...
    pub fn test_mean_dims_forward(
        ...
        let our = x.mean_dims(dims, keepdim);
        let data: Vec<f32> = (0..size).map(|i| (i as f32) * 0.05 - 1.0).collect();
        let torch_x = match LibTorchTensor::from_data(&data, shape) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        let torch = match torch_x.mean_dims(dims, keepdim) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        self.compare_tensors(&our, &torch)
    ...
    pub fn test_mean_dims_gradients(
        &self,
        shape: &[usize],
        dims: &[usize],
        keepdim: bool,
        ...
        let torch_m = match torch_x.mean_dims(dims, keepdim) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
...
mod tests {
    ...
    #[test]
    fn test_mean_validation_forward() {
        let v = TensorValidator::new(1e-6, 1e-8);
        let r = v.test_mean_forward(&[2, 3, 4]);
        assert!(r.passed, "mean forward validation failed: {}", r.details);
    }

    #[test]
    fn test_mean_validation_gradients_suites() {
        let v = TensorValidator::default();
        let shapes = vec![vec![6usize], vec![2, 3], vec![2, 2, 2]];
        for shape in shapes {
            let r = v.test_mean_forward(&shape);
            assert!(
                r.passed,
                "mean forward mismatch for {:?}: {}",
                shape, r.details
            );
            let g = v.test_mean_gradients(&shape);
            assert!(
                g.passed,
                "mean grad mismatch for {:?}: {}",
                shape, g.details
            );
        }
    }
    ...
    #[test]
    fn test_mean_dims_validation_suites() {
        let v = TensorValidator::default();
        let cases = vec![
        ...
        for (shape, dims, keepdim) in cases {
            let r = v.test_mean_dims_forward(&shape, &dims, keepdim);
            assert!(
                r.passed,
                "mean_dims forward mismatch for {:?} dims {:?} keepdim {}: {}",
                shape, dims, keepdim, r.details
            );
            let g = v.test_mean_dims_gradients(&shape, &dims, keepdim);
            assert!(
                g.passed,
                "mean_dims grad mismatch for {:?} dims {:?} keepdim {}: {}",
