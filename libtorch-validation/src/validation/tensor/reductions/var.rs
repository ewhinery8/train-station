...
impl TensorValidator {
    pub fn test_var_forward(&self, shape: &[usize]) -> ComparisonResult {
    ...
    pub fn test_var_gradients(&self, shape: &[usize]) -> ComparisonResult {
        let size: usize = shape.iter().product();
        ...
        let torch_v = match torch_y.var() {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        if let Err(e) = torch_v.backward(None) {
    ...
    pub fn test_var_dims_forward(
        ...
        let our = x.var_dims(dims, keepdim);
        let data: Vec<f32> = (0..size).map(|i| ((i % 7) as f32) - 3.0).collect();
        let torch_x = match LibTorchTensor::from_data(&data, shape) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        let torch = match torch_x.var_dims(dims, keepdim) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        self.compare_tensors(&our, &torch)
    ...
    pub fn test_var_dims_gradients(
        &self,
        shape: &[usize],
        dims: &[usize],
        keepdim: bool,
        ...
        let mut v = x.var_dims(dims, keepdim);
        v.backward(None);
        let our_grad = match x.grad_by_value() {
            Some(g) => g,
            None => return ComparisonResult::failure("Our grad missing".to_string()),
        ...
        let torch_v = match torch_x.var_dims(dims, keepdim) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        let grad_ones = match LibTorchTensor::ones(&torch_v.shape()) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
        if let Err(e) = torch_v.backward(Some(&grad_ones)) {
            return ComparisonResult::failure(e);
        }
        let torch_grad = match torch_x.grad() {
            Some(g) => g,
...
mod tests {
    ...
    #[test]
    fn test_var_validation() {
        let v = TensorValidator::default();
        let r = v.test_var_forward(&[2, 3]);
        assert!(r.passed, "var forward failed: {}", r.details);
        let g = v.test_var_gradients(&[2, 3]);
        assert!(g.passed, "var gradients failed: {}", g.details);
    }

    #[test]
    fn test_var_dims_validation_suites() {
        let v = TensorValidator::default();
        let cases = vec![
        ...
        for (shape, dims, keepdim) in cases {
            let r = v.test_var_dims_forward(&shape, &dims, keepdim);
            assert!(
                r.passed,
                "var_dims forward mismatch for {:?} dims {:?} keepdim {}: {}",
                shape, dims, keepdim, r.details
            );
            let g = v.test_var_dims_gradients(&shape, &dims, keepdim);
            assert!(
                g.passed,
                "var_dims grad mismatch for {:?} dims {:?} keepdim {}: {}",
