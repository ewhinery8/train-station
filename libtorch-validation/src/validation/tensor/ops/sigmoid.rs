...
impl TensorValidator {
    pub fn test_sigmoid_forward(&self, shape: &[usize]) -> ComparisonResult {
        ...
        let torch = match torch_x.sigmoid() {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
    ...
    pub fn test_sigmoid_gradients(&self, shape: &[usize]) -> ComparisonResult {
        let size: usize = shape.iter().product();
        ...
        let torch_y = match torch_x.sigmoid() {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
...
mod tests {
    ...
    #[test]
    fn test_sigmoid_validation_suites() {
        let v = TensorValidator::default();
        let shapes = vec![vec![3], vec![2, 3], vec![2, 3, 4]];
        for shape in shapes {
            let r = v.test_sigmoid_forward(&shape);
            assert!(
                r.passed,
                "sigmoid forward mismatch for {:?}: {}",
                shape, r.details
            );
            let g = v.test_sigmoid_gradients(&shape);
            assert!(
                g.passed,
                "sigmoid grad mismatch for {:?}: {}",
                shape, g.details
            );
        }
    }
