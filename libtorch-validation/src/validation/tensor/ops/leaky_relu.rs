...
impl TensorValidator {
    pub fn test_leaky_relu_forward(
        &self,
        shape: &[usize],
        negative_slope: f32,
    ) -> ComparisonResult {
        ...
        let our = x.leaky_relu(negative_slope);
        ...
        let torch = match torch_x.leaky_relu(negative_slope) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
    ...
    pub fn test_leaky_relu_gradients(
        &self,
        shape: &[usize],
        negative_slope: f32,
    ) -> ComparisonResult {
        ...
        let y = x.leaky_relu(negative_slope);
        let mut s = y.sum();
        s.backward(None);
        ...
        let torch_y = match torch_x.leaky_relu(negative_slope) {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
...
mod tests {
    ...
    #[test]
    fn test_leaky_relu_validation_suites() {
        let v = TensorValidator::default();
        let shapes = vec![vec![3], vec![2, 3], vec![2, 3, 4]];
        for shape in shapes {
            let r = v.test_leaky_relu_forward(&shape, 0.1);
            assert!(
                r.passed,
                "leaky_relu forward mismatch for {:?}: {}",
                shape, r.details
            );
            let g = v.test_leaky_relu_gradients(&shape, 0.1);
            assert!(
                g.passed,
                "leaky_relu grad mismatch for {:?}: {}",
                shape, g.details
            );
        }
    }
