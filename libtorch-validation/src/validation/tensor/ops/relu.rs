...
impl TensorValidator {
    pub fn test_relu_forward(&self, shape: &[usize]) -> ComparisonResult {
    ...
    pub fn test_relu_gradients(&self, shape: &[usize]) -> ComparisonResult {
        let size: usize = shape.iter().product();
        ...
        let torch_y = match torch_x.relu() {
            Ok(t) => t,
            Err(e) => return ComparisonResult::failure(e),
        };
...
mod tests {
    ...
    #[test]
    fn test_relu_validation_suites() {
        let v = TensorValidator::default();
        let shapes = vec![vec![3], vec![2, 3], vec![2, 3, 4]];
        for shape in shapes {
            let r = v.test_relu_forward(&shape);
            assert!(
                r.passed,
                "relu forward mismatch for {:?}: {}",
                shape, r.details
            );
            let g = v.test_relu_gradients(&shape);
            assert!(
                g.passed,
                "relu grad mismatch for {:?}: {}",
                shape, g.details
            );
        }
    }
